...
5. **ingredients** - Array of ingredients with exact quantities for ${people} people
6. **steps** - Array of 6-10 DETAILED step-by-step instructions. Each step should be a full paragraph (2-4 sentences) explaining the technique, what to look/smell for, timing details, and common mistakes to avoid.
7. **proTip** - A detailed pro tip (2-3 sentences)
8. **servingSuggestion** - What to serve alongside this dish
9. **youtubeSearch** - A YouTube search query string
10. **referenceUrl** - A search query for finding this recipe online
11. **isVegetarian** - boolean
12. **isGlutenFree** - boolean
...
[{"name":"...","time":"...","difficulty":"...","description":"...","ingredients":["..."],"steps":["..."],"proTip":"...","servingSuggestion":"...","youtubeSearch":"...","referenceUrl":"...","isVegetarian":true,"isGlutenFree":false,"spiceLevel":"medium"}]`;
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a professional Indian chef and recipe educator. Return ONLY valid JSON arrays, no markdown formatting, no code blocks, no extra text.",
          },
    ...
    // Hero images (best-effort — recipes still return if images fail)
    const images = await Promise.all(
      recipes.map((r: any) =>
        generateImage(
          `Beautiful appetizing food photography of the Indian dish "${r.name}". Top-down view on a rustic plate, warm lighting, garnished beautifully.`,
          API_KEY
        )
      )
    );
